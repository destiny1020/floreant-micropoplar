...
/**
 * This class is an abstract super class for all Xerces based parser/reg populator with
 * functionality to serialize an enumeration of JposEntry objects into XML
 * <p>
 * <b>NOTE</b>: this class must define a public no-argument ctor so that it may be created via
 * reflection when its defined in the jpos.properties as the jpos.config.regPopulatorClass
 * </p>
 * 
...
public abstract class AbstractXercesRegPopulator extends AbstractRegPopulator
  ...
  public void save(Enumeration entries) throws Exception {
    if (isPopulatorFileDefined())
      ...
      convertJposEntriesToXml(entries, new FileOutputStream(getDefaultXmlFileName()));
  }
  ...
  /**
   * @return the default XML file name that this populator will save entries to
   */
   ...
   * @since 1.2 (NY 2K meeting)
   */
  protected void serializeDocument(Document document, Enumeration entries, OutputStream os)
      throws Exception {
    ...
    try {
      ...
      tracer.println("Error making PrintWriter: " + "UnsupportedEncodingException.message = "
          + ex.getMessage());
    }
  ...
  /**
   * @return a String with the document type definition value. For DTD this would be the DTD
   *         relative path/file and for schemas the XSD relative path/file
   * @since 2.1.0
   */
  ...
  protected Document createEmptyDocument() {
    DOMImplementationImpl domImpImpl =
        (DOMImplementationImpl) DOMImplementationImpl.getDOMImplementation();
    DocumentType docType =
        domImpImpl.createDocumentType("JposEntries", "-//JavaPOS//DTD//EN", getDoctypeValue());
   ...
   * @since 1.2 (NY 2K meeting)
   */
  protected void appendCreationElement(Document doc, Element jposEntryElement,
      JposEntry jposEntry) {
   ...
   * @since 1.2 (NY 2K meeting)
   */
  protected void appendPropElement(Document doc, Element jposEntryElement, String propName,
      Object propValue) {
    jposEntryElement.appendChild(doc.createTextNode("    " + "    "));
  ...
  protected void appendPropElements(Document doc, Element jposEntryElement, JposEntry jposEntry) {
    ...
    while (props.hasMoreElements()) {
      ...
      if (!JposEntryUtility.isRequiredPropName(propName))
        appendPropElement(doc, jposEntryElement, propName, jposEntry.getPropertyValue(propName));
    }
  }
