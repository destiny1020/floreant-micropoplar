/*
 * JasperReports - Free Java Reporting Library. Copyright (C) 2001 - 2009 Jaspersoft Corporation.
 * All rights reserved. http://www.jaspersoft.com
 *
 * Unless you have purchased a commercial license agreement from Jaspersoft, the following license
 * terms apply:
 *
 * This program is part of JasperReports.
 *
 * JasperReports is free software: you can redistribute it and/or modify it under the terms of the
 * GNU Lesser General Public License as published by the Free Software Foundation, either version 3
 * of the License, or (at your option) any later version.
 *
 * JasperReports is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY;
 * without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Lesser General Public License for more details.
 * 
 * You should have received a copy of the GNU Lesser General Public License along with
 * JasperReports. If not, see <http://www.gnu.org/licenses/>.
 */
...
public class JRViewerPanel extends JPanel implements JRHyperlinkListener, JRViewerListener {
  ...
  protected void paintPage(Graphics2D grx) {
    ...
    try {
      ...
      exporter.setParameter(JRExporterParameter.PAGE_INDEX,
          Integer.valueOf(viewerContext.getPageIndex()));
  ...
  protected void refreshPage() {
    ...
    Dimension dim =
        new Dimension((int) (viewerContext.getJasperPrint().getPageWidth() * realZoom) + 8, // 2
                                                                                            // from
                                                                                            // border,
                                                                                            // 5
                                                                                            // from
  ...
  protected void setPageImage() {
    ...
    if (pageError) {
      ...
      try {
        image = JasperPrintManager.printPageToImage(viewerContext.getJasperPrint(),
            viewerContext.getPageIndex(), realZoom);
      } catch (Exception e) {
        if (log.isErrorEnabled()) {
        ...
        JOptionPane.showMessageDialog(this, java.util.ResourceBundle
            .getBundle("net/sf/jasperreports/view/viewer").getString("error.displaying"));
      }
    }
  ...
  protected Image getPageErrorImage() {
    Image image =
        new BufferedImage((int) (viewerContext.getJasperPrint().getPageWidth() * realZoom) + 1,
  ...
  protected class ImageMapPanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    protected JRPrintImageAreaHyperlink getImageMapArea(int x, int y) {
      ...
      if (imageAreaHyperlinks != null) {
        for (ListIterator it =
            imageAreaHyperlinks.listIterator(imageAreaHyperlinks.size()); image == null
                && it.hasPrevious();) {
  ...
  protected void upNavigate(KeyEvent evt) {
    if ((scrollPane.getViewport().getHeight() > pnlPage.getHeight()
        || scrollPane.getVerticalScrollBar().getValue() == 0) && viewerContext.getPageIndex() > 0) {
      viewerContext.setPageIndex(viewerContext.getPageIndex() - 1);
      if (scrollPane.isEnabled()) {
