...
/**
 * Mercury Payment Systems WebServices Platform Request <br />
 * <br />
 * ©2012 Mercury Payment Systems, LLC - all rights reserved. <br />
 * <br />
 * Disclaimer: <br />
 * This software and all specifications and documentation contained herein or provided to you
 * hereunder (the "Software") are provided free of charge strictly on an "AS IS" basis. No
 * representations or warranties are expressed or implied, including, but not limited to, warranties
 * of suitability, quality, merchantability, or fitness for a particular purpose (irrespective of
 * any course of dealing, custom or usage of trade), and all such warranties are expressly and
 * specifically disclaimed. Mercury Payment Systems shall have no liability or responsibility to you
 * nor any other person or entity with respect to any liability, loss, or damage, including lost
 * profits whether foreseeable or not, or other obligation for any cause whatsoever, caused or
 * alleged to be caused directly or indirectly by the Software. Use of the Software signifies
 * agreement with this disclaimer notice.
 *
...
public class MercuryWebRequest {
  ...
  private final String mSOAPWrapper =
      "<?xml version=\"1.0\" encoding=\"utf-8\"?><soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\"><soap:Body><%2$s xmlns=\"%1$s\">%3$s</%2$s></soap:Body></soap:Envelope>";
  private final String mMPSExceptionString = "MPSWebRequest Error: %1$s";

  /**
   * MercuryWebRequest Constructor
   * 
   * @param webServiceURL URL to MPS WebServices platform
   * @throws Exception When passed an invalid WebService URL
   */
   ...
   * @throws Exception When WebService URL does not use SSL
   */
   ...
   * 
   * @param timeout Web request timeout value in seconds
   * @throws Exception When timeout value is less than 1 second
   */
  public void setTimeout(int timeout) throws Exception {
    if (timeout > 0)
      ...
      throw new Exception(
          String.format(mMPSExceptionString, "Timeout value must be greater than 0"));
  }
   ...
   * @throws Exception When no parameters have been added to the MercuryWebRequest
   */
  private String buildSOAPRequest() throws Exception {
    if (!mWSParameters.isEmpty()) {
      ...
      throw new Exception(
          String.format(mMPSExceptionString, "Cannot build SOAP request with no parameters"));
    }
  }
   ...
   * 
   * @param paramName WebMethod parameter name
   * @param paramValue WebMethod parameter value
   */
  public void addParameter(String paramName, String paramValue) throws Exception {
    ...
    if (paramName.equals("tran"))
      paramValue = paramValue.replace("<", "&lt;").replace(">", "&gt;").replace("\t", "")
          .replace("\n", "").replace("\r", ""); // Remove less-than, greater-than, newline, carriage
                                                // return, and tab characters from value
   ...
   * @throws Exception When error is encountered communicating with WebService
   */
   ...
   * 
   * @throws Exception When missing required parameters or containing invalid parameters
   */
  private void validateRequiredParameters() throws Exception {
    ...
    if (!mWSParameters.containsKey("pw"))
      throw new Exception(String.format(mMPSExceptionString,
          "WebServices password parameter (\"pw\") is required"));
  }
}
