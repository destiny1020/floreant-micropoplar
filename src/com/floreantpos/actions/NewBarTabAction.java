...
public class NewBarTabAction extends AbstractAction implements CardInputListener {
  ...
  private void useSwipeCard(CardInputter inputter) {
    ...
    int option = JOptionPane.showOptionDialog(parentComponent, message, "Please confirm",
    ...
    try {
      ...
      String transactionId = CardConfig.getMerchantGateway().getProcessor().authorizeAmount(ticket,
          cardString, CardConfig.getBartabLimit(), selectedPaymentType.getDisplayString());
      ...
      ticket.addProperty(Ticket.PROPERTY_ADVANCE_PAYMENT,
          String.valueOf(CardConfig.getBartabLimit()));
  ...
  private void useManualCard(CardInputter inputter) {
    ...
    try {
      ...
      String transactionId = CardConfig.getMerchantGateway().getProcessor()
          .authorizeAmount(cardNumber, expMonth, expYear, CardConfig.getBartabLimit(), cardType);
