...
public class AuthorizeDotNetProcessor implements CardProcessor {
  ...
  public String authorizeAmount(Ticket ticket, String cardTracks, double amount, String cardType)
      throws Exception {
    ...
    // Create transaction
    Transaction authCaptureTransaction =
        merchant.createAIMTransaction(TransactionType.AUTH_ONLY, new BigDecimal(amount));
    authCaptureTransaction.setCreditCard(creditCard);

    Result<Transaction> result =
        (Result<Transaction>) merchant.postTransaction(authCaptureTransaction);
  ...
  public void authorizeAmount(PosTransaction transaction) throws Exception {
    ...
    // Create transaction
    Transaction authCaptureTransaction = merchant.createAIMTransaction(TransactionType.AUTH_ONLY,
        new BigDecimal(transaction.calculateAuthorizeAmount()));
    authCaptureTransaction.setCreditCard(creditCard);
  ...
  public String authorizeAmount(String cardNumber, String expMonth, String expYear, double amount,
  ...
  public void captureAuthorizedAmount(PosTransaction transaction) throws Exception {
    ...
    Transaction authCaptureTransaction = merchant.createAIMTransaction(
        TransactionType.PRIOR_AUTH_CAPTURE, new BigDecimal(transaction.getAmount()));
  ...
  public void captureNewAmount(PosTransaction transaction) throws Exception {
    ...
    // Create transaction
    Transaction authCaptureTransaction = merchant.createAIMTransaction(TransactionType.AUTH_CAPTURE,
  ...
  public void voidAmount(PosTransaction transaction) throws Exception {
    ...
    // Create transaction
    Transaction authCaptureTransaction = merchant.createAIMTransaction(TransactionType.VOID,
        new BigDecimal(transaction.calculateAuthorizeAmount()));
    authCaptureTransaction.setTransactionId(transaction.getCardTransactionId());
  ...
  public void voidAmount(String transId, double amount) throws Exception {
    ...
    // Create transaction
    Transaction authCaptureTransaction =
        merchant.createAIMTransaction(TransactionType.VOID, new BigDecimal(amount));
  ...
  private CreditCard createCard(PosTransaction transaction) {
    ...
    if (StringUtils.isNotEmpty(transaction.getCardTrack())) {
      ...
      return createCard(transaction.getCardNumber(), transaction.getCardExpiryMonth(),
          transaction.getCardExpiryYear(), transaction.getCardType());
    }
  }
  ...
  private CreditCard createCard(String cardNumber, String expMonth, String expYear,
      String cardType) {
    CreditCard creditCard = CreditCard.createCreditCard();
    creditCard.setCardType(CardType.findByValue(cardType));
  ...
  public static void main(String[] args) throws Exception {
    ...
    // createCard("%B4111111111111111^SHAH/RIAR^1803101000000000020000831000000?;4111111111111111=1803101000020000831?",
    // CardType.VISA.name());
    //
    // Transaction authTransaction = merchant.createAIMTransaction(TransactionType.AUTH_ONLY, new
    // BigDecimal(100));
    ...
    //
    // Transaction authCaptureTransaction =
    // merchant.createAIMTransaction(TransactionType.PRIOR_AUTH_CAPTURE, new BigDecimal(100));
    // authCaptureTransaction.setTransactionId(result.getTransId());
    // authCaptureTransaction.setCreditCard(creditCard);
    //
    // Result<Transaction> result2 = (Result<Transaction>)
    ...
    // Thread.sleep(1000);
    //
    // Transaction voidTransaction = merchant.createAIMTransaction(TransactionType.VOID, new
    // BigDecimal(100));
    // voidTransaction.setTransactionId(result.getTransId());
    // voidTransaction.setCreditCard(creditCard);
    //
    // Result<Transaction> result3 = (Result<Transaction>)
    // merchant.postTransaction(authCaptureTransaction);
    //
    ...
    final BankCardMagneticTrack track = BankCardMagneticTrack.from(
        "%B4111111111111111^SHAH/RIAR^1803101000000000020000831000000?;4111111111111111=1803101000020000831?");
