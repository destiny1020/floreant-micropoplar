...
public class TicketAuthorizationDialog extends POSDialog {
  ...
  private boolean confirmAuthorize(String message) {
    int option = JOptionPane.showConfirmDialog(TicketAuthorizationDialog.this, message, "Confirm",
  ...
  private void doAuthorize() {
    ...
    if (transactions == null || transactions.size() == 0) {
      POSMessageDialog.showMessage(TicketAuthorizationDialog.this,
          "Please select transactions to authorize");
      return;
    }
    ...
    PaymentProcessWaitDialog waitDialog =
        new PaymentProcessWaitDialog(TicketAuthorizationDialog.this);
  ...
  private void doEditTips() {
    ...
    final double newTipsAmount = NumberSelectionDialog2.show(TicketAuthorizationDialog.this,
        "Enter tips amount", oldTipsAmount);
  ...
  private void authorizeTransaction(PosTransaction transaction) throws Exception {
    ...
    if (StringUtils.isEmpty(cardEntryType)) {
      POSMessageDialog.showError("No input information found for transaction id = "
          + transaction.getId() + ". The record may be broken.");
      return;
    }
