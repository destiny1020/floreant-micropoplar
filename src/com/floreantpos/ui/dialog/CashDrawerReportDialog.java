...
public class CashDrawerReportDialog extends POSDialog {
  ...
  public void initialize() throws Exception {
    ...
    //
    // lblClosedTickets.setText(cashDrawerReport.getClosedTicketsSummary().getTotalTicket() + "
    // CLOSED TICKETS");
    //
    // lblClosedTicketsPrice.setText(currencySymbol +
    // decimalFormat.format(cashDrawerReport.getClosedTicketsSummary().getTotalPrice()));
    //
    // lblOpenTickets.setText(cashDrawerReport.getOpenTicketsSummary().getTotalTicket() + " OPEN
    // TICKETS");
    // lblOpenTicketsPrice.setText(currencySymbol +
    // decimalFormat.format(cashDrawerReport.getOpenTicketsSummary().getTotalPrice()));
  }
    ...
    btnPrint.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          ...
          POSMessageDialog.showError(CashDrawerReportDialog.this,
  ...
  private void btnResetCashDrawerActionPerformed() {// GEN-FIRST:event_btnResetCashDrawerActionPerformed
    int option = JOptionPane.showOptionDialog(this, "Sure reset cash drawer?", "Confirm",
  ...
  void addDiscountTableRow(Table table, String column1, String coulmn2) {
    TR tr = new TR();
    tr.addElement(
        new TD().addAttribute("style", "padding-left: 50px;").addElementToRegistry(column1));
    tr.addElement(new TD().addAttribute("align", "right").addElementToRegistry(coulmn2));
    table.addElement(tr);
  }

  void addExceptionTableRow(Table table, String column1, String coulmn2, String coulmn3,
      String coulmn4, String column5) {
    TR tr = new TR();
    TD td = new TD();
  ...
  public String createReport() throws Exception {
    ...
    addTableRow(table, "=GROSS RECEIPTS",
        decimalFormat.format(drawerPullReport.getGrossReceipts()));
    ...
    table = new Table();
    table.addAttribute("width", "100%");
    addTableRow(table, "-CASH RECEIPTS (" + drawerPullReport.getCashReceiptCount() + ")",
        decimalFormat.format(drawerPullReport.getCashReceiptAmount()));
    addTableRow(table, "-CREDIT CARDS  (" + drawerPullReport.getCreditCardReceiptCount() + ")",
        decimalFormat.format(drawerPullReport.getCreditCardReceiptAmount()));
    addTableRow(table, "-DEBIT CARDS   (" + drawerPullReport.getDebitCardReceiptCount() + ")",
        decimalFormat.format(drawerPullReport.getDebitCardReceiptAmount()));
    addTableRow(table, "-GIFT RETURNS  (" + drawerPullReport.getGiftCertReturnCount() + ")",
        decimalFormat.format(drawerPullReport.getGiftCertReturnAmount()));
    addTableRow(table, "+GIFT CERT. CHANGE",
        decimalFormat.format(drawerPullReport.getGiftCertChangeAmount()));
    addTableRow(table, "+CASH BACK", decimalFormat.format(drawerPullReport.getCashBack()));
    addTableSeparator(table);
    addTableRow(table, "=RECEIPT DIFFERENTIAL",
        decimalFormat.format(drawerPullReport.getReceiptDifferential()));
    document.appendBody(table);
    ...
    addTableSeparator(table);
    addTableRow(table, "=TIPS DIFFERENTIAL",
        decimalFormat.format(drawerPullReport.getTipsDifferential()));
    document.appendBody(table);
    ...
    table = new Table();
    table.addAttribute("width", "100%");
    addTableRow(table, "CASH  (" + drawerPullReport.getCashReceiptCount() + ")",
        decimalFormat.format(drawerPullReport.getCashReceiptAmount()));
    // addTableRow(table, "CASH TAX", decimalFormat.format(drawerPullReport.getCashTax()));
    addTableRow(table, "-TIPS PAID", decimalFormat.format(drawerPullReport.getTipsPaid()));
    addTableRow(table, "-PAY OUT       (" + drawerPullReport.getPayOutCount() + ")",
        decimalFormat.format(drawerPullReport.getPayOutAmount()));
    addTableRow(table, "-CASH BACK", decimalFormat.format(drawerPullReport.getCashBack()));
    addTableRow(table, "+BEGIN CASH", decimalFormat.format(terminal.getOpeningBalance()));
    addTableRow(table, "-DRAWER BLEED  (" + drawerPullReport.getDrawerBleedCount() + ")",
        decimalFormat.format(drawerPullReport.getDrawerBleedAmount()));
    addTableSeparator(table);
    addTableRow(table, "=DRAWER ACCOUNTABLE",
        decimalFormat.format(drawerPullReport.getDrawerAccountable()));
    addTableRow(table, ">CASH TO DEPOSIT",
        decimalFormat.format(drawerPullReport.getCashToDeposit()));
    document.appendBody(table);
    ...
    if (voidTickets != null) {
      for (DrawerPullVoidTicketEntry entry : voidTickets) {
        addExceptionTableRow(table, String.valueOf(entry.getCode()), entry.getReason(), " ",
            String.valueOf(entry.getQuantity()), NumberUtil.formatNumber(entry.getAmount()));
      }
    }
    addExceptionTableSeparator(table);
    document.appendBody(table);
    ...
    table = new Table();
    table.addAttribute("width", "100%");
    addTableRow(table, "TOTAL VOIDS W/WST",
        decimalFormat.format(drawerPullReport.getTotalVoidWst()));
    ...
    addDiscountTableRow(table, "TOTAL COUNT",
        String.valueOf(drawerPullReport.getTotalDiscountCount()));
    addDiscountTableRow(table, "TOTAL Dsct",
    ...
    addDiscountTableRow(table, "Check Size",
        String.valueOf(drawerPullReport.getTotalDiscountCheckSize()));
