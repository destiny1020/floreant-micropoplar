...
public class DataMigrationWindow extends JFrame {
  ...
  protected void migrate() {
    ...
    sourceConfiguration =
        sourceConfiguration.setProperty("hibernate.dialect", sourceDatabase.getHibernateDialect());
    sourceConfiguration = sourceConfiguration.setProperty("hibernate.connection.driver_class",
        sourceDatabase.getHibernateConnectionDriverClass());

    sourceConfiguration =
        sourceConfiguration.setProperty("hibernate.connection.url", sourceConnectString);
    sourceConfiguration =
        sourceConfiguration.setProperty("hibernate.connection.username", sourceDbPanel.getUser());
    sourceConfiguration =
        sourceConfiguration.setProperty("hibernate.connection.password", sourceDbPanel.getPass());

    Configuration destConfiguration = _RootDAO.getNewConfiguration(null);

    destConfiguration =
        destConfiguration.setProperty("hibernate.dialect", destDatabase.getHibernateDialect());
    destConfiguration = destConfiguration.setProperty("hibernate.connection.driver_class",
        destDatabase.getHibernateConnectionDriverClass());

    destConfiguration =
        destConfiguration.setProperty("hibernate.connection.url", destConnectString);
    destConfiguration =
        destConfiguration.setProperty("hibernate.connection.username", destDbPanel.getUser());
    destConfiguration =
        destConfiguration.setProperty("hibernate.connection.password", destDbPanel.getPass());
