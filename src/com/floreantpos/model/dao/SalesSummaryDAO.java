...
public class SalesSummaryDAO extends _RootDAO {
  ...
  public List<SalesAnalysisData> findSalesAnalysis(Date start, Date end, UserType userType,
      Terminal terminal) {
    ...
    try {
      ...
      if (terminal != null) {
        criteria.add(Restrictions.eq("item." + TicketItem.PROP_TICKET + "." + Ticket.PROP_TERMINAL,
            terminal));
      }
        ...
        for (MenuCategory category : categories) {
          ...
          if (terminal != null) {
            criteria.add(Restrictions
                .eq("item." + TicketItem.PROP_TICKET + "." + Ticket.PROP_TERMINAL, terminal));
          }
          datas = criteria.list();
          if (datas.size() > 0) {
            Object[] objects = (Object[]) datas.get(0);
  ...
  public SalesStatistics findKeyStatistics(Date start, Date end, UserType userType,
      Terminal terminal) {
    Session session = null;
    ...
    try {
      ...
      if (restaurant != null) {
        salesSummary.setCapacity(
            restaurant.getCapacity() != null ? restaurant.getCapacity().intValue() : 0);
        salesSummary
            .setTables(restaurant.getTables() != null ? restaurant.getTables().intValue() : 0);
      }
...

      {
        ...
        for (Object object : list) {
          ...
          if (!attendenceHistory.isClockedOut() || attendenceHistory.getClockOutTime() == null) {
            ...
            laborHourInMillisecond =
                Math.abs(end.getTime() - attendenceHistory.getClockInTime().getTime());
  ...
  private void findRecordByProfitCenter(Date start, Date end, UserType userType, Terminal terminal,
      Session session, SalesStatistics salesSummary, Shift shift, TicketType ticketType) {
    Criteria criteria;
