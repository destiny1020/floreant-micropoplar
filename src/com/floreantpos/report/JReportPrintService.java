...
public class JReportPrintService {
  ...
  public static void printGenericReport(String title, String data,
      Map<String, String> additionalCtx) throws Exception {
    ...
    JasperPrint jasperPrint = createJasperPrint(
        "/com/floreantpos/report/template/GenericReport.jasper", map, new JREmptyDataSource());
  ...
  public static JasperPrint createPrint(Ticket ticket, Map<String, String> map,
      PosTransaction transaction) throws Exception {

    final String FILE_RECEIPT_REPORT =
        "/com/floreantpos/report/template/TicketReceiptReport.jasper";
  ...
  public static void printRefundTicket(Ticket ticket, RefundTransaction posTransaction) {
    try {
      ...
      TicketPrintProperties printProperties =
          new TicketPrintProperties("*** 退款收据 ***", true, true, true);
      printProperties.setPrintCookingInstructions(false);
  ...
  public static void printTransaction(PosTransaction transaction) {
    try {
      ...
      TicketPrintProperties printProperties =
          new TicketPrintProperties("*** 支付收据 ***", true, true, true);
      printProperties.setPrintCookingInstructions(false);
  ...
  public static HashMap populateTicketProperties(Ticket ticket,
      TicketPrintProperties printProperties, PosTransaction transaction) {
    ...
    if (printProperties.isShowFooter()) {
      ...
      map.put("serviceChargeText",
          POSConstants.RECEIPT_REPORT_SERVICE_CHARGE_LABEL + currencySymbol);
  ...
  private static StringBuilder buildTicketHeader(Ticket ticket,
      TicketPrintProperties printProperties) {
    ...
    beginRow(ticketHeaderBuilder);
    addColumn(ticketHeaderBuilder, POSConstants.RECEIPT_REPORT_TERMINAL_LABEL
        + Application.getInstance().getTerminal().getId());
    endRow(ticketHeaderBuilder);
    ...
    beginRow(ticketHeaderBuilder);
    addColumn(ticketHeaderBuilder,
        POSConstants.RECEIPT_REPORT_DATE_LABEL + DateUtil.getReceiptDate(new Date()));
    endRow(ticketHeaderBuilder);
    ...
    if (ticket.getType() != TicketType.DINE_IN) {
      ...
      if (StringUtils.isNotEmpty(customerName)) {
        ...
        if (ticket.getDeliveryDate() != null) {
          beginRow(ticketHeaderBuilder);
          addColumn(ticketHeaderBuilder,
              "Delivery: " + DateUtil.getReceiptDate(ticket.getDeliveryDate()));
          endRow(ticketHeaderBuilder);
        }
      }
    }
